(function () {
  ...
  const els = {
    ...
    authLoginIdentifierText: document.getElementById("authLoginIdentifierText"),
  ...
  function openAuthDialog(mode) {
    ...
    els.authLoginIdentifierText.textContent = mode === "create" ? "Email" : "Username or Email";
    els.authUsernameInput.value = "";
    els.authEmailInput.value = "";
    els.authEmailInput.type = mode === "create" ? "email" : "text";
    els.authEmailInput.autocomplete = mode === "create" ? "email" : "username";
  ...
  async function submitAuth() {
    ...
    const identifier = normalizeLoginIdentifier(els.authEmailInput.value);
    ...
    const validation = validateCredentials({
      ...
      email,
      password,
      requireUsername: state.authMode === "create",
      requireEmail: state.authMode === "create",
    });
    if (validation) {
  ...
  async function createAccount(username, email, password) {
    ...
    if (Object.values(accounts).some((account) => usernameKey(account.username) === usernameKey(username))) {
  ...
  async function login(email, password) {
    ...
    if (!loginEmail) {
      els.authStatus.textContent = "Username or email is incorrect.";
      return;
    }
    ...
    if (supabaseClient) {
      ...
      if (error) {
        els.authStatus.textContent = getErrorMessage(error, "Username/email or password is incorrect.");
        return;
      }
    ...
    const account = getAccounts()[loginEmail];
    if (!account || account.password !== password) {
      els.authStatus.textContent = "Username/email or password is incorrect.";
      return;
    }
  ...
  async function resolveLoginEmail(identifier) {
    ...
    if (supabaseClient) {
      const { data, error } = await supabaseClient.rpc("email_for_username", { login_username: value });
      if (!error && data) return normalizeEmail(Array.isArray(data) ? data[0] : data);

      return "";
    }

    const accounts = getAccounts();
    const match = Object.entries(accounts).find(([, account]) => usernameKey(account.username) === value);
  ...
  function validateCredentials({ username, identifier, email, password, requireUsername, requireEmail }) {
    ...
    if (!identifier) return requireEmail ? "Enter an email." : "Enter a username or email.";
    if (requireEmail && !isValidEmail(email)) return "Enter a valid email.";
  ...
  async function exportImage() {
    ...
    const drawPile = (pile, x, y) => {
      ...
      const countText = String(pile.total);
      const countX = x + sectionHeaderW - 22;
      const countY = y + sectionHeaderH / 2;
      const countBadgeR = 14;
      ctx.beginPath();
      ctx.arc(countX, countY, countBadgeR, 0, Math.PI * 2);
      ctx.fillStyle = "rgba(8, 10, 12, 0.42)";
      ctx.fill();
      ctx.fillStyle = "#fff";
      ctx.textAlign = "left";
      ctx.textBaseline = "middle";
      ctx.font = pile.key === "main-character" ? "900 16px system-ui, sans-serif" : "900 18px system-ui, sans-serif";
      ctx.fillText(pile.label, x + 18, y + sectionHeaderH / 2);
      ctx.textAlign = "right";
      ctx.font = "900 17px system-ui, sans-serif";
      ctx.fillText(countText, countX + 5, countY);
  ...
  function createRestSupabaseClient(baseUrl, anonKey) {
    ...
    return {
      ...
      async rpc(name, args = {}) {
        return requestJson(`/rest/v1/rpc/${name}`, { method: "POST", body: JSON.stringify(args) });
      },
    };
  }
  ...
  function loadPermalinkDeck() {
    const payload = new URLSearchParams(location.search).get("deck");
    if (!payload) return;

    try {
      const decoded = JSON.parse(fromBase64(payload));
      const imported = new Map();
      Object.entries(decoded.deck || {}).forEach(([rawId, rawQty]) => {
        const id = resolveCardId(rawId);
        const qty = Math.max(0, Math.floor(Number(rawQty) || 0));
        if (id && qty > 0) imported.set(id, qty);
      });
      if (!imported.size) return;

      state.deck.clear();
      state.deckName = String(decoded.name || "").trim() || "Untitled Deck";
      state.author = normalizeAuthor(decoded.author) || "anonymous";
      imported.forEach((qty, id) => {
        const card = cardById.get(id);
        for (let index = 0; index < qty; index += 1) {
          if (!card || !canAddCard(card)) break;
      ...
      // Ignore malformed shared links and keep the locally saved deck.
    }
  }
