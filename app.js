(function () {
  ...
  function renderAccount() {
    ...
    if (isLoggedIn) {
      els.userMenuBtn.textContent = state.currentUser;
    }
  }
  ...
  function saveCurrentDeckToLibrary() {
    if (!state.currentUser) {
  ...
  function login(username, password) {
    ...
    if (!account || account.password !== password) {
      els.authStatus.textContent = "Username or password is incorrect.";
      return;
    }
    setCurrentUser(username);
    els.authDialog.close();
  ...
  function deleteAccount() {
    ...
    setAccounts(accounts);
    localStorage.removeItem(savedDecksKey(username));
  ...
  function savedDecksKey(username) {
    return `${SAVED_DECKS_PREFIX}${username}`;
  }
