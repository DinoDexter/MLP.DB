(function () {
  ...
  async function saveCurrentDeckToLibrary() {
    ...
    if (supabaseClient && state.currentUserId) {
      const { data, error } = await supabaseClient
        ...
        .single();
      if (error) {
        els.saveDeckStatus.textContent = error.message || "Could not save deck.";
        return;
      }
  ...
  async function deleteSelectedSavedDeck() {
    ...
    if (supabaseClient && state.currentUserId) {
      ...
      if (error) {
        showText("Delete Deck", error.message || "Could not delete deck.");
        return;
      }
    }
  ...
  async function createAccount(username, email, password) {
    if (supabaseClient) {
      ...
      if (error) {
        els.authStatus.textContent = error.message || "Could not create account.";
        return;
      }
  ...
  async function login(email, password) {
    if (supabaseClient) {
      ...
      if (error) {
        els.authStatus.textContent = error.message || "Email or password is incorrect.";
        return;
      }
  ...
  function validateCredentials({ username, email, password, requireUsername }) {
    ...
    if (password.length < 4 || password.length > 20) return "Password must be 4 to 20 characters.";
    return "";
  }
  ...
  function createRestSupabaseClient(baseUrl, anonKey) {
    ...
    const authHeaders = (useUserAuth = true, extra = {}) => ({
      apikey: anonKey,
    ...
    const requestJson = async (path, options = {}) => {
      const response = await fetch(`${baseUrl}${path}`, {
        ...options,
        headers: {
          ...authHeaders(options.auth !== false, { "Content-Type": "application/json" }),
          ...(options.headers || {}),
        },
      });
